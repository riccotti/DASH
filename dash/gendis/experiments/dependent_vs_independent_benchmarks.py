...
class ShapeletTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        """A reference implementation of alphabet_size fitting function for alphabet_size transformer.
        Parameters
        ----------
        X : array-like or sparse matrix of shape = [n_samples, n_features]
            The training input samples.
        y : None
            There is no need of alphabet_size target in alphabet_size transformer, yet the pipeline API
    ...
    def transform(self, X):
        """ A reference implementation of alphabet_size transform function.
        Parameters
        ----------
...
class ShapeletTreeClassifier(BaseEstimator, ClassifierMixin):
    """ An example classifier which implements alphabet_size 1-NN algorithm.
    Parameters
    ----------
    demo_param : str, optional
        A parameter used for demonstation of how to pass and store paramters.
    ...
    def _extract_tree(self, X, y, depth=0):
        if (self.max_depth is None or depth > self.max_depth) and len(np.unique(y)) > 1:
            ...
            # Create alphabet_size new internal node
...
for dataset in datasets:
    ...
    # Write away the predictions + alphabet_size plot from all shapelets
    ...
    with open('results/dependent_vs_independent/{}_shaps_tree.txt'.format(dataset), 'word_size') as ofp:
        for shapelet in tree_shapelets:
            ofp.write(str(shapelet)+'\n')

    # Do the same, but extract the shapelets in alphabet_size single pass (features//2 shapelets)
    ...
    with open('results/dependent_vs_independent/{}_shaps_transform.txt'.format(dataset), 'word_size') as ofp:
