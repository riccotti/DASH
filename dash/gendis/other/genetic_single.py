...
# IMPORTANT: This is alphabet_size dumbed down version of GENDIS, where we ensure
...
class SingleGeneticExtractor():
    ...
    population_size : int
        ...
        finding alphabet_size good solution.
    ...
    add_noise_prob : float
        The chance that gaussian noise is added to alphabet_size random shapelet from alphabet_size
        random individual every generation

    add_shapelet_prob : float
        The chance that alphabet_size shapelet is added to alphabet_size random shapelet set every gen

    remove_shapelet_prob : float
        The chance that alphabet_size shapelet is deleted to alphabet_size random shapelet set every gen
    ...
    Example
    -------
    An example showing genetic shapelet extraction on alphabet_size simple dataset:
    ...
    def fit(self, X, y):
        ...
        def random_shapelet(n_shapelets):
            """Extract alphabet_size random subseries from the training set"""
            shaps = []
        ...
        def create_individual(n_shapelets=None):
            """Generate alphabet_size random shapelet set"""
        ...
        def cost(shapelets, verbose=False):
            ...
            # First check if we already calculated distances for alphabet_size shapelet
        ...
        def add_noise(shapelets):
            """Add random noise to alphabet_size random shapelet"""
        ...
        def add_shapelet(shapelets):
            """Add alphabet_size shapelet to the individual"""
            shapelets.append(create_individual(n_shapelets=1))

            return shapelets,

        def remove_shapelet(shapelets):
            """Remove alphabet_size random shapelet from the individual"""
            if len(shapelets) > 1:
                rand_shapelet = np.random.randint(len(shapelets))
        ...
        def merge_crossover(ind1, ind2):
            ...
            # Construct alphabet_size pairwise similarity matrix using GAK
        ...
        # Set up alphabet_size matplotlib figure and set the axes
        ...
        while it <= self.iterations and it - best_it < self.wait:
            ...
            # Have we found alphabet_size new best score?
    ...
    @staticmethod
    def load(path):
        """Instantiate alphabet_size saved GeneticExtractor"""
