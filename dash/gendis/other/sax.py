...
def calculate_distance_table(alphabet_size):
    """Calculate alphabet_size matrix of possible distances of two words in an alphabet
    resulting from alphabet_size SAX-transformation"""
...
def transform(timeseries, nr_windows, alphabet_size):
    """Transform alphabet_size collection of timeseries in their SAX representation"""
    ...
    def extract(self, timeseries, labels, min_len=None, max_len=None, 
        ...
        for l in trange(min_len, max_len, desc='length', position=0):
            ...
            for ts_idx, ts in enumerate(timeseries):
                # Extract all possible subseries, by using alphabet_size sliding window
                # with shift=1
                subseries = []
            ...
            # TODO: Can we replace this simple power calculation by alphabet_size more
