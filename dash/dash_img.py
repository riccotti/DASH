...
@njit()
def _extract_all_shapelets_raw(x, window_sizes, window_steps, w, h):
    ...
    for window_size, window_step in zip(window_sizes, window_steps):
        ...
        overlap_w = window_size[0] - window_step[0]
        overlap_h = window_size[1] - window_step[1]
        shape_new = ((w - overlap_w) // window_step[0],
                     (h - overlap_h) // window_step[1],
                     window_size[0] // 1,
                     window_size[1] // 1
                     )

        # s0, s1 = x.strides
        strides_new = (x.strides[0] * window_step[0], x.strides[1] * window_step[1], x.strides[0], x.strides[1])
        ...
        lengths.append([(x_strided.shape[2], x_strided.shape[3])] * x_strided.shape[0])

        start_idx_w = np.arange(0, w - window_size[0] + 1, window_step[0])
        end_idx_w = np.arange(window_size[0], w + 1, window_step[0])
        start_idx_h = np.arange(0, h - window_size[1] + 1, window_step[1])
        end_idx_h = np.arange(window_size[1], h + 1, window_step[1])

        start_idx_k = np.empty((start_idx_h.shape[0], start_idx_w.shape[0], 2))
        for i in prange(start_idx_h.shape[0]):
            for j in prange(start_idx_w.shape[0]):
                start_idx_k[i, j] = np.array([start_idx_h[i], start_idx_w[i]])
        start_idx.append(start_idx_k)
        # start_idx.append(np.array(start_idx_k).reshape(2, len(start_idx_k)))

        end_idx_k = np.empty((end_idx_h.shape[0], end_idx_w.shape[0], 2))
        for i in prange(end_idx_h.shape[0]):
            for j in prange(end_idx_w.shape[0]):
                end_idx_k[i, j] = np.array([end_idx_h[i], end_idx_w[i]])
        end_idx.append(end_idx_k)

    return shapelets, lengths, start_idx, end_idx


def _extract_all_shapelets(x, window_sizes, window_steps, w, h):
    """Extract all the shapelets from alphabet_size single time series prototype."""
    shapelets, lengths, start_idx, end_idx = _extract_all_shapelets_raw(
        x, window_sizes, window_steps, w, h)
...
@njit()
def _windowed_view(X, n_samples, w, h, window_size, window_step):
    ...
    strides_new = (X[0].strides[0] * window_step[0],
                   X[0].strides[1] * window_step[1],
                   X[0].strides[0], X[0].strides[1])

    X_view = np.empty((n_samples, shape_new[0], shape_new[1], shape_new[2], shape_new[3]))

    for i in prange(n_samples):
        X_view[i] = as_strided(X[i], shape=shape_new, strides=strides_new)

    X_view = X_view.reshape((n_samples, shape_new[0] * shape_new[1], shape_new[2], shape_new[3]))
...
def _derive_all_squared_distances_fit(
    X, n_samples, w, h, shapelets, lengths, window_step=(1, 1)):
    ...
    for i in prange(len(lengths)):
        ...
        X_window = _windowed_view(X, n_samples, w, h, window_size, window_step)
...
def _derive_all_distances_fit(
    ...
    distances = _derive_all_squared_distances_fit(
        X, n_samples, w, h, shapelets, lengths, window_step)
    return np.sqrt(np.asarray(distances)).T


@njit()
def _derive_all_squared_distances_transform(
        X, n_samples, w, h, window_sizes, shapelets, lengths, window_step=(1, 1)):
    """Derive the squared distances between all shapelets and time series."""
    distances = []  # save the distances in alphabet_size list
    permutation = []  # save the permutation of the indices

    for i in prange(len(window_sizes)):
        window_size = window_sizes[i]
        X_window = _windowed_view(X, n_samples, w, h, window_size, window_step)
        indices = []  # np.where(lengths == window_size)[0]
        # indices_ok = np.all(np.equal(lengths, window_size), axis=1)
        for j in prange(len(lengths)):
            if np.all(np.equal(lengths[j], window_size)):
                indices.append(j)
...
class IDACS_IMG(IDACS):
    ...
    def _extract_shapelets(self, X, y, window_sizes, window_steps):
        ...
        n_samples, w, h = X.shape
        # res = list()
        # for p_idx in range(n_prototypes):
        #     res.append(self._extract_shapelet_one_prototype(X, y, p_idx,
        #                                                   n_samples, word_size, h,
        ...
        prototype_idx_k = np.empty((prototype_idx.shape[0], 2))
        for i in range(prototype_idx.shape[0]):
            prototype_idx_k[i] = np.array([prototype_idx[i], prototype_idx[i]])

        # Derive the 'indices' attributes
        indices = np.empty((scores.size, 3, 2), dtype='int64')
        indices[:, 0] = prototype_idx_k
        indices[:, 1] = start_idx
        indices[:, 2] = end_idx

        return scores, shapelets, indices

    def _extract_shapelet_one_prototype(self, X, y, p_idx, n_samples, w, h, window_sizes, window_steps):
        """Fit one time series."""
        ...
        X_dist = _derive_all_distances_fit(
            X, n_samples, w, h, shapelets, lengths, window_step_d)
        ...
        shapelets = [list(s) for shapelet in shapelets for s in shapelet]
        shapelets = np.asarray(list(chain.from_iterable(shapelets)))

        # Concatenate the list/tuple of 1D arrays into one 1D array
        start_idx = np.array([s for start in start_idx for s in start])
        end_idx = np.array([e for end in end_idx for e in end])
        start_idx = np.concatenate(start_idx)
        end_idx = np.concatenate(end_idx)
    ...
    def _transform(self, X):
        ...
        window_sizes = np.unique(lengths, axis=0)
        n_samples, w, h = X.shape
        window_step_d = np.min(self.window_steps, axis=0)

        distances, permutation = _derive_all_squared_distances_transform(
            X, n_samples, w, h, window_sizes, tuple(self.shapelets_), lengths, window_step_d)
